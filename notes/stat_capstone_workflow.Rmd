...
library(visreg)   #visualization regression models 
library(pROC)
library(viridisLite)   
library(viridis)       
```
...
points_cu <- points_cu %>%
  ...
  mutate(
    ...
    # Assign a unique run ID whenever the rally result switches (win to loss)
...
```{r}
#distribution 
ggplot(pbp2, aes(x = d_pre)) +
  geom_histogram(bins = 30, fill = "#440154FF", color = "white") +
  labs(
    title = "Distribution of Pre-Rally Score Differential",
    x = "Score Differential (CU - Opponent)",
    y = "Count"
  ) +
  theme_minimal()
```
...
```{r}
#Independence of Observations (check for potential clustering)
table(pbp2$match_id)
table(pbp2$set_number)  
```
...
#only test continuous predictors, so exclude factors like cu_serv
...
# # of predictors (excluding intercept)
...
OTHER MODEL FOR RALLY-LEVEL PROBABILITY PREDICTIONS.
##Basic GAM Model
*Use points_final dataset for modeling*.
```{r}
#predictors
...
##5-Fold CV Model Validation for GAM Contd.
```{r}
...
ggplot(calib, aes(phat, ybar, size=n)) +
  geom_point(alpha=.85,color="#4B0082") +
...
sets_df <- pbp2 %>%
  group_by(match_id, set_number) %>%
  # take the final rally row for each set 
  slice_tail(n = 1) %>%
  ungroup() %>%
  mutate(
    pred_set_win = mapply(get_set_win_prob,
...
#Compute Brier & Log-loss using only complete cases
...
```{r}
#Simulation-based validation: simulate full sets 
...
##RESULTS
Final graphics and plots to display results effectively. 

#rally probability 
```{r}
#Empirical rally win rate by (d_pre, cu_serv)
emp_df <- points_tomod %>%
  ...
  mutate(
    Serve = if_else(cu_serv == 1L, "CU Serve", "Opposition Serve")
  )

#GAM predictions over smooth grid
d_range <- range(points_tomod$d_pre, na.rm = TRUE)

pred_df <- expand.grid(
  d_pre = seq(d_range[1], d_range[2], length.out = 300),
  cu_serv = c(0L, 1L)
) %>%
  mutate(
    pred_prob = as.numeric(
      predict(m_point_gam,
              newdata = data.frame(d_pre = d_pre,
                                   cu_serv = cu_serv),
              type = "response")
    ),
    Serve = if_else(cu_serv == 1L, "CU Serve", "Opposition Serve")
...
ggplot() +
  ...
  geom_point(
    data = emp_df,
    aes(x = d_pre, y = emp_prob, color = Serve, size = n),
    alpha = 0.7
  ) +
  # GAM predictions
  geom_line(
    data = pred_df,
    aes(x = d_pre, y = pred_prob, color = Serve),
    linewidth = 1.1
  ) +
  scale_color_manual(
    values = c("CU Serve" = "#4B0082",   # dark purple
               "Opposition Serve" = "#FFD700")  # volleyball yellow
  ) +
  scale_size_continuous(name = "Number of Observations") +
  xlim(-10, 10) +
  ylim(0.00, 1.00) +
  labs(
    title = "Rally Win Probability vs Score Differential",
    subtitle = "GAM Model vs Empirical Data",
    x = "Score Differential",  #Score Differential (CU - Opp) pre-rally
    y = "Probability CU Wins Rally",   #P(CU wins rally)
    color = "Serve"
  ) +
...
ggplot(grid30_GAM, aes(x = opp, y = cu, fill = prob)) +
  geom_tile() +
  facet_wrap(~ cu_serve) +
  scale_fill_viridis_c(name = "CU Set Win Probability", limits = c(0,1)) +
  coord_equal() +
  labs(x = "Opponent Points", y = "CU Points",
       title = "Set Win Probabilities Across Score States by Serving Team") +
  theme_minimal(base_size = 12)
...
ggplot(band, aes(x = opp, y = cu)) +
  ...
  coord_equal() +
  labs(x = "Opponent Points", y = "CU Points",
       title = "Deuce Band (22–30): Set Win Probabilities by Serving Team") +
  theme_minimal(base_size = 12)
```
...
ggplot(points2, aes(x = cu_score_end, y = opp_score_end, fill = leverage)) +
  ...
  labs(title = "Change in CU Set Win Probability",
       x = "Opponent Score", y = "CU Score", fill = "Change In Win Probability") +
  coord_equal()
#rally leverage 
```

```{r}
#Build grid of states + swing values from Markov chain
build_swing_grid <- function(max_score = 30L) {
  expand.grid(
    cu  = 0:max_score,
    opp = 0:max_score,
    srv = c(0L, 1L)          # 0 = opp serve, 1 = CU serve
    ...
    mutate(
      cu_serve   = if_else(srv == 1L, "CU serve", "Opp serve"),
      term       = mapply(is_terminal, cu, opp),

      # next states if CU wins / loses the next rally
      cu_win_cu   = pmin(cu + 1L, max_score),
      cu_win_opp  = opp,
      cu_win_srv  = 1L,                      # CU serves after winning
      cu_lose_cu  = cu,
      cu_lose_opp = pmin(opp + 1L, max_score),
      cu_lose_srv = 0L,                      # opp serves after losing

      p_now  = V[cbind(cu + 1L, opp + 1L, srv + 1L)],
      p_win  = V[cbind(cu_win_cu + 1L, cu_win_opp + 1L, cu_win_srv + 1L)],
      p_lose = V[cbind(cu_lose_cu + 1L, cu_lose_opp + 1L, cu_lose_srv + 1L)],

      swing_raw = p_win - p_lose,
      swing     = if_else(term, NA_real_, swing_raw)  # keep NA for terminal states
    )
}

swing_grid <- build_swing_grid(30L)


#Net Probability Swing of the Next Rally
max_swing <- max(swing_grid$swing, na.rm = TRUE)

swing_grid_plot <- swing_grid %>%
  mutate(
    swing_plot = if_else(is.na(swing), 0, swing)  # NA → 0 so they draw in dark purple
  )
```

#Net Probabilty Swing 
```{r}
# 1) Find the max non-missing swing value
max_swing <- max(swing_grid$swing, na.rm = TRUE)

# 2) Modify ONLY the small top-right 29–30 block
swing_grid_fix <- swing_grid %>%
  mutate(
    swing_plot = case_when(
      cu >= 29 & opp >= 29 ~ max_swing,   # top-right corner → bright yellow
      TRUE ~ swing                          # everything else stays exactly as before
    )
  )

# 3) Plot with corrected corner highlight
ggplot(swing_grid_fix, aes(x = opp, y = cu, fill = swing_plot)) +
  geom_tile() +
  facet_wrap(~ cu_serve) +
  scale_fill_viridis_c(
    name = "Two-Sided Swing",
    option = "viridis",
    limits = c(0, max_swing),
    na.value = "#440154"   # deep purple for NA – matches original viridis low end
  ) +
  coord_equal() +
  labs(
    title = "Next Rally Net Probability Swing",
    x = "Opponent Points",
    y = "CU Points"
  ) +
  theme_minimal(base_size = 14)
...
```{r}
#Cluster swing values into 5 tiers (non-terminal states only)
set.seed(22)
nonterm <- swing_grid %>%
  filter(!term, !is.na(swing))

# k-means on the swing values (5 clusters)
k_fit <- kmeans(nonterm$swing, centers = 5, nstart = 50)

nonterm <- nonterm %>%
  mutate(cluster = k_fit$cluster)

# cluster summaries: avg swing, avg gain, avg loss
cluster_summary <- nonterm %>%
  mutate(
    gain = p_win  - p_now,
    loss = p_now  - p_lose
  ) %>%
  group_by(cluster) %>%
  summarise(
    mean_swing = mean(swing),
    mean_gain  = mean(gain),
    mean_loss  = mean(loss),
    .groups    = "drop"
  ) %>%
  arrange(desc(mean_swing)) %>%        # C1 = highest swing
  mutate(
    cluster_id = row_number(),
    label = paste0(
      "C", cluster_id,
      " · swing=", round(mean_swing, 3)
      # " · gain=",  round(mean_gain, 3),
      # " · loss=",  round(mean_loss, 3)
    )
  )

# map numeric cluster -> ordered factor with pretty labels
plot_df <- nonterm %>%
  left_join(cluster_summary %>% select(cluster, cluster_id), by = "cluster") %>%
  mutate(
    cluster_f = factor(
      cluster_id,
      levels = sort(unique(cluster_id)),                         # 1..5
      labels = cluster_summary$label[order(cluster_summary$cluster_id)]
    )
  )

# Get label corresponding to C1 (the highest-swing cluster)
label_C1 <- levels(plot_df$cluster_f)[1]

plot_df_fix <- plot_df %>%
  mutate(
    cluster_f = as.character(cluster_f),
    cluster_f = if_else(cu >= 29 & opp >= 29, label_C1, cluster_f),
    cluster_f = factor(cluster_f, levels = levels(plot_df$cluster_f))
  )

#reverse viridis so C1 = bright yellow, C5 = dark purple
cluster_colors <- rev(viridis(5))

ggplot(plot_df_fix, aes(x = opp, y = cu, fill = cluster_f)) +
  geom_tile() +
  facet_wrap(~ cu_serve) +
  scale_fill_manual(
    values = cluster_colors,
    name   = expression(Delta * " Win Probabilities")
  ) +
  coord_equal() +
  labs(
    title = "Clusters of Next-Rally Probability Swing Across Score States",
    x = "Opponent Points",
    y = "CU Points"
  ) +
...
points_final %>%
  ...
  geom_histogram(binwidth = 1, fill = "#440154", color = "black", alpha = 0.7) +
...
ggplot(points_balanced, aes(x = cu_score_end, fill = deciding_set)) +
  ...
  scale_fill_manual(values = c("#440154", "yellow"),
                    labels = c("Non-Deciding Set", "Deciding Set")) +
  scale_color_manual(values = c("purple", "lightyellow"), guide = "none") +
  labs(
    title = "Distribution of CU Points at Set End",
...
OTHER TWO FOR LOGISTIC + GAM are with there respective code sections.Rest of overall results graphics at the top.
