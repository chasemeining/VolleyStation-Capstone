...
library(pROC)
library(viridisLite)   # for viridis colors
...
grid30_GAM %>%
  ...
  #scale_fill_viridis_c()+  WANT COLORS FROM THIS PALLETE FOR GRAPHIC FIX FOR ME 
  scale_fill_manual(
    values = c("#440154FF", "steelblue", "lightgreen", "yellow"),
...
```{r}
# 3. Cluster swing values into 5 tiers (non-terminal states only)
set.seed(22)
...
# Colors: reverse viridis so C1 = bright yellow, C5 = dark purple
cluster_colors <- rev(viridis(5))


ggplot(plot_df, aes(x = opp, y = cu, fill = cluster_f)) +
  geom_tile() +
  facet_wrap(~ cu_serve) +
  scale_fill_manual(
...
# ---- Force the top-right corner (cu >= 29 & opp >= 29) into C1 (yellow) ----
# Get the label corresponding to C1 (the highest-swing cluster)
