...
##MAIN DATA CLEANING 
#add encoding set type: non-deciding (to 25) vs. deciding (to 15) + match state: which set number (1–5), sets won/lost so far.
```{r}
#data inflow + outflow
#in_path  <- "all_matches"
...
```{r}
#SAME AS CODE 153-on  but with added variables
#collapse touches into rally-level dataset
points_core <- touches_flagged %>%
  group_by(match_id, set_number, rally_id) %>%
  summarise(
...
```{r}
#SAME AS 255-on , but DIFFERENT BY JUST ADDING FEW MORE VARIABLES 
points_final <- points_cu %>%
  left_join(set_scores, by = c("match_id", "set_number")) %>%
  select(
    match_id, date, season, conference,
    ...
    touches_in_rally,
    timeout_after_point,
    substitution_after_point,    # NEW
    #home_team_won_rally,         # NEW, same as winning point (same as cu_point_won, except by home_team)
    won_match_rally              # NEW, whether the home team eventually won the match that the rally belongs to
  )
```
...
## I THINK WE SHOULD BE CONCISE IN NAMING VARIABLES someting_point , something_rally : should choose one to be consisent 
